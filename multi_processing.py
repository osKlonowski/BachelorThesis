...
import numpy as np
from write_results import write_header, write_result
from classes.solution_result import SolutionResult
import time
import random
import asyncio
...
def get_avg_fitness(result):
    avg_fitness = 0
    for solution in result:
        avg_fitness += solution.fitness
    return avg_fitness / len(result)


def construct_write_csv_result(res, time):
    data = []
...
async def compute(listOfSections):
    write_header()
    max_core_count = multiprocessing.cpu_count()
    print(f'Using {max_core_count} cores')
    inputs = generateInputs(listOfSections)
    pool = multiprocessing.Pool(max_core_count)
    started = time.time()
    res = pool.starmap(compute_section, inputs)
    elapsed = time.time()
    processed = list(divide_chunks(res, 3))
    for attempt in processed:
        construct_write_csv_result(attempt, elapsed - started)
    print('\n\nTime - MultiProcessing Took :', elapsed - started)
    pool.close()
    pool.join()
...
# //['avg_fitness', 'time', 'num_iter', 'num_ants', 'alpha', 'beta', 'Q']
